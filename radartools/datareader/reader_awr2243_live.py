...
from ..client.radar_client import RadarClient

class LiveAWR2243Reader():
    """Live AWR2243 data reader class.
    
    NOTE: Make sure this is started after the capture process if you want the latest index files.
          According to my test, the server will only read the latest index files and after,
          so if the server is up before the new capture progress started, the received data will be 
          the last 2nd index files and after.
    """

    def __init__(self, server_dir, nloops, nsamples, radarip, radar_client :RadarClient=None, serverport=18888, 
        ...
        Args:
            ...
            radarip (str): The IP address of the server.
        ...
        # isrunning, errmsg = self.is_server_running(self.radarip, self.serverport, self.connect_timeout)
        # if not isrunning:
        #     raise ConnectionError(errmsg)
        

    def nextframe(self, query_timeout=2) -> np.ndarray | None:
        """Send query to the client and retrive frame.
        
        Returns:
            np.ndarray: The received frame.
        """
        if self.radar_client is not None:
            frame = self.__query_frame_until_EOF(self.radar_client.server_stdin, self.radar_client.server_stdout, self.radar_client.server_stderr)
        else:
            frame = self.__query_frame_until_timeout(query_timeout)
        # TODO save frame
        return frame
    
    def __query_frame_until_EOF(self, server_stdin: ChannelStdinFile, server_stdout: ChannelFile, server_stderr: ChannelStderrFile) -> np.ndarray:
        """Query the frame with radar client stdout and stderr until EOF.

        Returns:
            np.ndarray: The received frame.
        """
        _ = server_stdin
        backline = server_stdout.readline()
        # if backline == "":
        #     pass
        # elif 
    ...
    def __query_frame_until_timeout(self, query_timeout) -> np.ndarray | None:
        """Query the frame until timeout."""
        ...
        while len(recv_data) < self.expected_data_length:
            try:
                ...
                if not packet:
                    is_server_running, errmsg = self.is_server_running(self.radarip, self.serverport, query_timeout)
    ...
    def is_server_running(self, radarip, serverport, timeout) -> tuple[bool, str]:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            return_value = s.connect_ex((radarip, serverport))
            if return_value == 0:
                return True, 'Connection established.'
            elif return_value == 11:
                errmsg = (f'Connection timeout, please check if the port {serverport} on {radarip} is open.')
            elif return_value == 111:
                errmsg = (f'Connection refused, please check if the port {serverport} on {radarip} is in use.')
            elif return_value == 113:
                errmsg = (f'No route to host, please make sure the server {radarip} exists and can be reached.')
            else:
                # see https://github.com/torvalds/linux/blob/master/include/uapi/asm-generic/errno.h for the meaning of error codes
                errmsg = (f'Cannot connect to {radarip}, socket error {return_value}.')
    ...
    def __get_socket(self, reconnect=False) -> socket.socket:
        ...
        """
        if self.read_client is None or reconnect == True:
            # create a socket object
            try:
                self.read_client = socket.create_connection((self.radarip, self.serverport), timeout=self.connect_timeout)
            except socket.timeout:
                raise TimeoutError('Timeout when connecting to the server.') from None # avoid nested exception
            except ConnectionRefusedError:
                raise ConnectionRefusedError('Connection refused, please check if the port is in use.') from None # avoid nested exception
            except Exception as e:
                raise e from None
            # set input buffer size
            self.read_client.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, self.expected_data_length*3)
            # set output buffer size
            self.read_client.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, self.expected_data_length)

            # test connection
            frame_idx = 1 # TODO 
            config_msg = f"{self.server_dir},{frame_idx},{self.nsamples},{self.nchirps_one_loop},{self.nloops},{self.rx_enable},1000"
            self.read_client.sendall(config_msg.encode())
            recv_start_flag = self.read_client.recv(1)
